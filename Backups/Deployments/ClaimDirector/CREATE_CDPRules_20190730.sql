...
FROM
	...
	INNER JOIN ClaimSearch_Prod.dbo.CLT00004 WITH (NOLOCK)
		ON UnpivotedReasonCodes.I_ALLCLM = CLT00004.I_ALLCLM
			AND CLT00004.C_ROLE = 'IN'
	LEFT OUTER JOIN ClaimSearch_Prod.dbo.CLT00007 WITH (NOLOCK)
		ON UnpivotedReasonCodes.I_ALLCLM = CLT00007.I_ALLCLM
			AND CLT00004.I_NM_ADR = CLT00007.I_NM_ADR
	LEFT OUTER JOIN ClaimSearch_Prod.dbo.CLT0007A WITH(NOLOCK)
		ON CLT00007.I_ALLCLM = CLT0007A.I_ALLCLM
			AND CLT00007.I_NM_ADR = CLT0007A.I_NM_ADR		
	...
	LEFT OUTER JOIN ClaimSearch_Prod.dbo.CLT0008A WITH(NOLOCK)
		ON CLT00008.I_ALLCLM = CLT0008A.I_ALLCLM
		AND CLT00008.I_NM_ADR = CLT0008A.I_NM_ADR
	CROSS APPLY (
		SELECT
			CASE
