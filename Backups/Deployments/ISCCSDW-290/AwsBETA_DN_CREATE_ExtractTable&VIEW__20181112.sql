...
*/
CREATE TABLE DecisionNet.ClaimReferenceExtractAWSBeta
(
	...
	*/
	...
	dateBilledRun DATE NULL, /*D_BILL_RUN CAST dateBilledRun fact table*/
	dateFilled DATE NULL, /*D_FILL CAST dateFilled from fact table*/
	/*DevNote:*/
	...
	*/
	...
	isLocationSearchUsed BIT NOT NULL, /*[LOCATION SEARCH ENTRY] contrived/derived from fact*/
	isPersonalSearchUsed BIT NOT NULL, /*[PERSONAL SEARCH ENTRY]	contrived/derived from fact*/
	isVehicleSearchUsed BIT NOT NULL, /*[VEHICLE SEARCH ENTRY] contrived/derived from fact*/
...
************************************************/
CREATE VIEW DecisionNet.V_ClaimReferenceExtractAWSBeta AS
(
	SELECT
		ClaimReferenceExtractAWSBeta.transactionId AS I_TRNS,
		ClaimReferenceExtractAWSBeta.userId AS I_USR,
		ClaimReferenceExtractAWSBeta.claimReferenceNumber,
		ClaimReferenceExtractAWSBeta.transactionDate AS D_TRNS,
		ClaimReferenceExtractAWSBeta.companySoldToCode,
		CompanyHeirarchy.Customer_lvl0 + ' - ' + ClaimReferenceExtractAWSBeta.companySoldToCode AS companyName,
		ClaimReferenceExtractAWSBeta.officeSoldToCode,
		ClaimReferenceExtractAWSBeta.companyShippedToCode,
		ClaimReferenceExtractAWSBeta.officeShippedToCode,
		ClaimReferenceExtractAWSBeta.productTransactionCode,
		ClaimReferenceExtractAWSBeta.dateBilledRun,
		ClaimReferenceExtractAWSBeta.dateFilled,
		ClaimReferenceExtractAWSBeta.originalBillFlag AS F_BILLABLE,
		ClaimReferenceExtractAWSBeta.alternateBillFlag AS F_BILLMATCH,
		ClaimReferenceExtractAWSBeta.userName AS NAME,
		/*ClaimReferenceExtract.userCustomerCode AS I_CUST_USR,*/
		/*ClaimReferenceExtract.userOfficeCode AS I_REGOFF_USR,*/
		ClaimReferenceExtractAWSBeta.userJobClassDescription AS Jobclass_Desc,
		...
		*/
		ClaimReferenceExtractAWSBeta.productGroupName AS productCategory,
		1 AS volume,
		CASE
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId IS NULL
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 1 /*People*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 2 /*Vehicle_Location_Products*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 3 /*Assets*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 4 /*Business*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 5 /*Healthcare*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 6 /*Directory_Assistance*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 7 /*Weather_Reports*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 8 /*Criminal_Records*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 9 /*Police_Reports*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 10 /*Driver_History*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 11 /*Medical_Records*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 12 /*ClaimDirector*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		CASE
			...
			THEN
				NULL
			WHEN
				ClaimReferenceExtractAWSBeta.productGroupId = 13 /*OTHER*/
			THEN
				CAST(1 AS INT)
			ELSE
				0
		...
		0 AS A_LN_ITM_EXTN_TR,
		ClaimReferenceExtractAWSBeta.unitTax AS A_LN_ITM_TAX_TR,
		ClaimReferenceExtractAWSBeta.invoiceDate,
		ClaimReferenceExtractAWSBeta.invoiceNumber,
		ClaimReferenceExtractAWSBeta.productCode,
		/*Columns Deprecated
			ClaimReferenceExtract.vendorTransactionDescription AS [T_VEND_TRNS],
			ClaimReferenceExtract.vendorAccountType AS [ACT_TYP],
		*/
		ClaimReferenceExtractAWSBeta.vendorId
	...
	FROM
		DecisionNet.ClaimReferenceExtractAWSBeta  WITH(NOLOCK)
		LEFT OUTER JOIN dbo.V_MM_Hierarchy AS CompanyHeirarchy WITH(NOLOCK)
			ON CompanyHeirarchy.lvl0 = ClaimReferenceExtractAWSBeta.companySoldToCode
	WHERE
		/*Refactor to DATEFROMPARTS in SQLSERVER 2012,
			use of BETWEEN preservs potential indexes on the DATE column*/
		ClaimReferenceExtractAWSBeta.transactionDate >= CAST(CAST((YEAR(GETDATE())-4) AS CHAR(4)) +'0101' AS DATE)
		AND ClaimReferenceExtractAWSBeta.companySoldToCode IN
		(
			'E257',
			'T014',
			'L005',
