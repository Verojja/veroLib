		...
		IF(
			...
			(
				...
				WHERE
					...
					AND DATEDIFF(HOUR,GETDATE(),AdjusterActivityLog.executionDateTime) < 12
			)
		)
		BEGIN
...
SELECT *
  FROM [ClaimSearch_Prod].[dbo].[CLT00002]
  where I_ALLCLM = '7A003999485'
    order by n_adj_seq asc
  
  
  /****** Script for SelectTopNRows command from SSMS  ******/
SELECT *
  FROM [ClaimSearch_Prod].[dbo].[CLT00004]
  where I_ALLCLM = '7A003999485'
  order by I_NM_ADR asc