...
SELECT *
FROM dbo.FireMarshalController
WHERE FireMarshalController.endDate IS NULL AND projectedGenerationDate IS NOT NULL;
...
END
GO
/*One Time update for forcing delta to capture current month. Also Updates Quarterly to observe 05 day generation-cycle*/

UPDATE dbo.FireMarshalController
	SET
		FireMarshalController.projectedGenerationDate = CAST('2019-04-05' AS DATE)
WHERE
	FireMarshalController.projectedGenerationDate = CAST('2019-08-01' AS DATE)
	AND FireMarshalController.endDate IS NULL;
UPDATE dbo.FireMarshalController
	SET
		FireMarshalController.projectedGenerationDate = CAST('2019-06-05' AS DATE)
WHERE
	FireMarshalController.projectedGenerationDate = CAST('2019-07-01' AS DATE)
	AND FireMarshalController.endDate IS NULL;
GO
IF(@@TRANCOUNT <1 OR @@ERROR <> 0)
BEGIN
...
Description: Logging table for FireMarshal Generation
				
			Performance: No current notes.
************************************************/
CREATE TABLE dbo.FireMarshalGenerationLog
(
	fireMarshalGenerationLogId BIGINT IDENTITY(1,1) NOT NULL,
	...
	CONSTRAINT PK_FireMarshalGenerationLog_fireMarshalGenerationLogId
		PRIMARY KEY CLUSTERED (fireMarshalGenerationLogId)
);
GO
IF(@@TRANCOUNT <1 OR @@ERROR <> 0)
BEGIN
...
END
GO
CREATE NONCLUSTERED INDEX NIX_FireMarshalGenerationLog_isSuccessful_stepId_executionDateTime
	ON dbo.FireMarshalGenerationLog (isSuccessful, stepId, executionDateTime);
GO
IF(@@TRANCOUNT <1 OR @@ERROR <> 0)
BEGIN
	ROLLBACK TRANSACTION;
...
************************************************/
CREATE PROCEDURE dbo.hsp_FireMarshalSendClaims
...
FROM
	...
	INNER JOIN (
		SELECT
			InnerFireMarshalClaimSendHistory.lossStateCode,
			MAX(InnerFireMarshalClaimSendHistory.fireMarshallDate) AS mostRecentDate
		FROM
			dbo.FireMarshalClaimSendHistory AS InnerFireMarshalClaimSendHistory
		WHERE
			InnerFireMarshalClaimSendHistory.reportingStatus = 'Sent'
		GROUP BY
			InnerFireMarshalClaimSendHistory.lossStateCode
	) AS MaxDateSendHistory
		ON FireMarshalClaimSendHistory.lossStateCode = MaxDateSendHistory.lossStateCode
			AND FireMarshalClaimSendHistory.fireMarshallDate = MaxDateSendHistory.mostRecentDate
WHERE
	FireMarshalClaimSendHistory.reportingStatus = 'Sent'
