...
BEGIN TRANSACTION
DROP TABLE dbo.FireMarshalController
...
(
	...
	*/,
	...
	CONSTRAINT PK_FireMarshalController_fmStateCode_dateInserted
		PRIMARY KEY CLUSTERED (fmStateCode, dateInserted)
);
GO
IF(@@TRANCOUNT <1 OR @@ERROR <> 0)
BEGIN
...
END
GO
CREATE UNIQUE INDEX FUK_FireMarshalController_fmStateCode_endDate
	ON dbo.FireMarshalController (fmStateCode, endDate)
	WHERE FireMarshalController.endDate IS NULL;
GO
IF(@@TRANCOUNT <1 OR @@ERROR <> 0)
BEGIN
	ROLLBACK TRANSACTION;
...
SELECT
	CAST(UniqueInstanceStateStatus.[state] AS CHAR(2)) AS fmStateCode,
...
FROM
	...
	LEFT OUTER JOIN (
		VALUES
			...
			('GA', 'DOUGLAS', 'R', 'BURDICK', NULL, 'PROPERTY INSURANCE LOSS REGISTER', NULL, '700 NEW BRUNSWICK AVENUE', NULL, 'RAHWAY', 'NJ', '7065', NULL, 'MR', /*reqSetId*/10, 0, 1, 0, /*frequencyCode*/ 'Q'),
	...
	) AS FMContactListValueSet (fmStateCode, firstName, middleName, lastName, suffixName, dept, div, fMDeliveryAddressLine1, fMDeliveryAddressLine2, fMDeliveryCity, fMDeliveryStateCode, zipCode, title, salutation, fmQualificationRequirmentSetId, receivesPrint, receivesFTP, receivesEmail, frequencyCode)
		ON UniqueInstanceStateStatus.[state] = FMContactListValueSet.fmStateCode
WHERE
...
SELECT
	#FireMarshalControllerDataToInsert.fmStateCode,
