...
ALTER PROCEDURE dbo.hsp_FireMarshalSendClaims
	...
	@executionDateParam DATE = NULL
AS
BEGIN
	BEGIN TRY
		SELECT /*Initialization of DEFAULT-NULL executionDate-param*/
			@executionDateParam = COALESCE(@executionDateParam,GETDATE());
		...
		IF(
			...
			(
				...
				WHERE
					...
					AND MONTH(FireMarshalGenerationLog.executionDateTime) = MONTH(@executionDateParam)
			)
			AND DAY(@executionDateParam) < 5
		)
		BEGIN
			SELECT
				@sourceDateTime	= CASE
					WHEN
						CAST(@executionDateParam AS DATE) <> CAST(GETDATE() AS DATE)
					THEN
						@executionDateParam
					ELSE
					COALESCE(
		...
		BEGIN
			...
			FROM
				...
				LEFT OUTER JOIN dbo.FireMarshalGenerationLog
					ON 1 = 1 /*HACK*/
						AND FireMarshalGenerationLog.stepId = 100 /*One Sided Filter*/
						AND FireMarshalGenerationLog.isSuccessful = 1 /*One Sided Filter*/;
		...
		/*Set Logging Variables for Current Step_End_Success*/
			...
			WHERE
				...
				AND ISNULL(FireMarshalController.projectedGenerationDate,CAST('99990101' AS DATE))<= CAST(@executionDateParam AS DATE)
