	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT
				...
				NULLIF(LTRIM(RTRIM(DashboardAggregationByClaim.adjusterName)),'') AS adjusterName,
			...
			FROM
				(
					SELECT
						...
						NULLIF(LTRIM(RTRIM(V_ActiveAdjuster.adjusterName)),'') AS adjusterName,
						NULLIF(
							LTRIM(
								RTRIM(
									CAST(
										V_ActiveAdjuster.adjusterAreaCode AS CHAR(3)
									)
								)
							),
							'0'
						...
						+ NULLIF(
							LTRIM(
								RTRIM(
									CAST(
										V_ActiveAdjuster.adjusterPhoneNumber AS CHAR(4)
									)
								)
							),
							'0'
					...
					FROM
						...
						LEFT OUTER JOIN dbo.V_ActiveAdjuster
							ON V_ActiveCurrentPendingFMClaim.adjusterId = V_ActiveAdjuster.adjusterId
			...
			FROM
				(
					SELECT
						...
						(
							...
							ORDER BY
								...
								CASE
									WHEN
										NULLIF(LTRIM(RTRIM(V_ActiveAdjuster.adjusterName)),'') IS NOT NULL
									THEN
										0
									ELSE
										1
						...
						COALESCE(V_ActiveLocationOfLoss.scrubbedAddressLine1, V_ActiveLocationOfLoss.originalAddressLine1, V_ActiveFireMarshalClaimSendHistory.lossAddressLine1) AS lossAddressLine1,
						/*V_ActiveFireMarshalClaimSendHistory.lossAddressLine2,*/
						COALESCE(V_ActiveLocationOfLoss.scrubbedAddressLine2, V_ActiveLocationOfLoss.originalAddressLine2, V_ActiveFireMarshalClaimSendHistory.lossAddressLine2) AS lossAddressLine2,
						/*V_ActiveFireMarshalClaimSendHistory.lossCityName,*/
						COALESCE(V_ActiveLocationOfLoss.scrubbedCityName,V_ActiveLocationOfLoss.originalCityName, V_ActiveFireMarshalClaimSendHistory.lossCityName) AS lossCityName,
						...
						COALESCE(V_ActiveLocationOfLoss.scrubbedZipCodeExtended,V_ActiveLocationOfLoss.scrubbedZipCode,V_ActiveLocationOfLoss.originalZipCode, V_ActiveFireMarshalClaimSendHistory.lossZipCode) AS lossZipCode,
						...
						COALESCE(V_ActiveLocationOfLoss.scrubbedCountyFIPS,V_ActiveFireMarshalClaimSendHistory.lossGeoCountyFipsCode) AS lossGeoCountyFipsCode,
						NULLIF(LTRIM(RTRIM(V_ActiveAdjuster.adjusterName)),'') AS adjusterName,
						NULLIF(
							LTRIM(
								RTRIM(
					...
					FROM
						...
						LEFT OUTER JOIN dbo.V_ActiveClaim
							ON V_ActiveFireMarshalClaimSendHistory.claimId = V_ActiveClaim.claimId
						LEFT OUTER JOIN dbo.V_ActiveLocationOfLoss
							ON V_ActiveClaim.locationOfLossAddressId = V_ActiveLocationOfLoss.addressId
						LEFT OUTER JOIN dbo.V_ActiveAdjuster
