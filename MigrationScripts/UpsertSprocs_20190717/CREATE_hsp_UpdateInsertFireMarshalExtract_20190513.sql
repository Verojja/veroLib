		...
		BEGIN
			...
			UPDATE dbo.FireMarshalExtract
				SET
					...
					FireMarshalExtract.dateInserted = SOURCE.dateInserted
			FROM
			...
			WHERE
				...
				(
					...
					OR FireMarshalExtract.isActive <> 1
			...
			SELECT
				@stepId = 100,
				@stepDescription = 'Delete Stale FMExtract record',
				@stepStartDateTime = GETDATE();
				
			DELETE FROM dbo.FireMarshalExtract
			WHERE
				NOT EXISTS (
					SELECT
						NULL
