...
/*
TODO:
	1.) find&repalce "AdjusterActivityLog" with current object activity log table.
...
************************************************/
CREATE PROCEDURE dbo.hsp_UpdateInsertAdjuster
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT
				...
				DuplicateDataSetPerformanceHack.adjusterName,
				...
				(/*Notes on DuplicateDataSetPerformanceHack: dbo.Adjuster contains duplicate records
					...
					FROM
						...
						INNER JOIN ClaimSearch_Prod.dbo.CLT00002 WITH (NOLOCK)
							ON FM_ExtractFile.I_ALLCLM = CLT00002.I_ALLCLM
						INNER JOIN ClaimSearch_Prod.dbo.CLT00014 WITH (NOLOCK)
							ON CLT00002.I_ALLCLM = CLT00014.I_ALLCLM
			...
			*/
			...
			WHERE
				...
				AND 
				(
					Adjuster.adjusterCompanyCode <> ISNULL(SOURCE.adjusterCompanyCode,'')
					OR Adjuster.adjusterOfficeCode <> ISNULL(SOURCE.adjusterOfficeCode,'')
					OR Adjuster.adjusterDateSubmitted <> ISNULL(SOURCE.adjusterDateSubmitted,'')
					OR Adjuster.adjusterName <> ISNULL(SOURCE.adjusterName,'')
					OR Adjuster.adjusterAreaCode <> ISNULL(SOURCE.adjusterAreaCode,-1)
					OR Adjuster.adjusterPhoneNumber <> ISNULL(SOURCE.adjusterPhoneNumber,-1)
					OR Adjuster.isoClaimId <> ISNULL(SOURCE.isoClaimId,'')
					OR Adjuster.involvedPartySequenceId <> ISNULL(SOURCE.involvedPartySequenceId,-1)
					OR Adjuster.adjusterSequenceId <> ISNULL(SOURCE.adjusterSequenceId,-1)
				);
